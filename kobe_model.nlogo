...
__includes [
  "evacu_fanction_kobe.nls"
  "movement.nls"
  "shortest_algorithm_kobe.nls"
  "actions_kobe.nls"
  "fileoperation_kobe.nls"
  "kobeparam.nls"
]

to loopsimulation
  let loopcnt 0
  while [loopcnt < 3][ ;;3 times loop
    setup
    while [timeunit <= 3600 * 18] ;;18 hours
    [
      go
      ;;set timeunit timeunit + 1
      ;;set hours floor (timeunit / 3600)
      ;;set minutes floor ((timeunit mod 3600) / 60)
      ;;set seconds timeunit mod 60
      ;;set zerominutes "00"
      ;;set zeroseconds "00"
      ...
      ;;if (timeunit mod 600) = 0
     ;; [
     ;; statisticsl hours minutes loopcnt
      ;;if (timeunit mod 1800) = 0 [
...
to setup
  ...
  resize-world 0 (1620 * 1 - 1) 0 (1080 * 1 - 1) ;;10km/1620=6.2m
  reset-timer
  load_maps
  ;;set_evac
  show timer
...
to go
  ;;if count turtles with [agent-type = 0] = 0 [
  ;;  statistics3
  ;;  stop
  ;;]
  ;;if timeunit >= 3600 * 18 [
  ...
  ask turtles with [hasinfo? = false and moveway = 0 and agent-type = 0][
  ;;if hasinfo? = false and moveway = 0 [
    if isevac? = true [
    if waitmove <= 0 [
        while [waitmove <= 0][
         set previous_x xcor
         set previous_y ycor
         movetoguide
          if moved? = false [movetogether]
          if moved? = false [randommove2]
         let distancefromprevious max list 1 (distancexy previous_x previous_y)
         set waitmove waitmove + movespeedinv * distancefromprevious
         if distancefromprevious < 0.5 [set waitmove 0.001]
       ]
  ...
  ask turtles with [hasinfo? = false and moveway = 1 and agent-type = 0][
    if isevac? = true [
    if waitmove <= 0 [
      while [waitmove <= 0][
        set previous_x xcor
        ...
        movetoguide
          if moved? = false [movetogether]
        if moved? = false [moveforward 800]
        let distancefromprevious max list 1 (distancexy previous_x previous_y)
        set waitmove waitmove + movespeedinv * distancefromprevious
        if distancefromprevious < 0.5 [set waitmove 0.001]
      ]
  ...
  ask turtles with [hasinfo? = false and moveway = 2 and agent-type = 0][
  ;;if hasinfo? = false and moveway = 2 [
    if isevac? = true [
      if waitmove <= 0 [
      while [waitmove <= 0][
        set previous_x xcor
  ...
  ask turtles with [hasinfo? = false and moveway = 3 and agent-type = 0][
    if isevac? = true [
      if waitmove <= 0 [
      while [waitmove <= 0][
        set previous_x xcor
  ...
  ;;]
  ask_information
  if timeunit >= 3600 * 4 [guide_action]
  send_information


  ;;set agentnum 0
  ;;ask turtles [ set agentnum agentnum + peoplenum]
  ...
  set timeunit timeunit + 1
  set hours 6 + floor (timeunit / 3600)
  set minutes floor ((timeunit mod 3600) / 60)
  set seconds timeunit mod 60
  set zerominutes "00"
  set zeroseconds "00"
...
"" ""
PENS
"hasinfo" 1.0 0 -5298144 true "" "plot rednum"
